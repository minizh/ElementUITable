<template>
  <div>
    <el-table
      ...
      @row-click="rowClick"
      @expand-change="expandChageFn"
    >
      <el-table-column prop="date" label="日期" width="180"> </el-table-column>
      <el-table-column prop="name" label="姓名" width="180">
        <template slot-scope="scope">
          <el-button type="primary" @click="add(scope.row, $event)"
            >添加</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    add(row, event) {
      console.log(event);
      if (row.loaded && row.expanded) {
        return;
      }
      if (!row.hasChild) {
        this.$set(row, "loaded", false);
        this.$set(row, "expanded", true);
        this.$set(row, "hasChild", true);
      }
      this.$nextTick(() => {
        event.target.offsetParent.previousElementSibling
          .querySelector(".el-table__expand-icon")
          .click();
      });
      // const temp = [
      //   {
